...
export const Chart = () => {
    ...
    const minBinIdPrice = useMemo(() => {
        return binIdToPrice({
            binId: minBinId,
            binStep: new Decimal(activePosition?.associatedLiquidityPool?.dlmmState?.binStep ?? 0),
            basisPointMax: new Decimal(activePosition?.associatedLiquidityPool?.dlmmState?.basisPointMax ?? 0),
            decimalsA: new Decimal(tokenA?.decimals ?? 0),
            decimalsB: new Decimal(tokenB?.decimals ?? 0),
        })
    }, [minBinId, activePosition?.associatedLiquidityPool?.dlmmState?.binStep, activePosition?.associatedLiquidityPool?.dlmmState?.basisPointMax, tokenA?.decimals, tokenB?.decimals])
    ...
    }, [maxBinId, activePosition?.associatedLiquidityPool?.dlmmState?.binStep, activePosition?.associatedLiquidityPool?.dlmmState?.basisPointMax, tokenA?.decimals, tokenB?.decimals])
    ...
    }, [activeId, activePosition?.associatedLiquidityPool?.dlmmState?.binStep, activePosition?.associatedLiquidityPool?.dlmmState?.basisPointMax, tokenA?.decimals, tokenB?.decimals])
        ...
        <div>
            <div className="grid place-items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                    <div className="text-xs">
                        {round(minBinIdPrice)} - {round(maxBinIdPrice)}{" "}
